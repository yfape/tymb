...
class AipNlp extends AipBase {
     ...
     * @return array
     */
    public function ecnet($textarr, $options=array()){
        $resarr = [];
        foreach ($textarr as $key => $value) {
            ...
            $data = array_merge($data, $options);
            $data = mb_convert_encoding(json_encode($data), 'GBK', 'UTF8');
            $res = $this->request($this->ecnetUrl, $data);
            if(isset($res['item']['vec_fragment'])){
                foreach ($res['item']['vec_fragment'] as $k => $v) {
                    array_push($resarr,[
                        'error_text'=>$v['ori_frag'],
                        'correct_text'=>$v['correct_frag']
                    ]);
                }
            }
        }
